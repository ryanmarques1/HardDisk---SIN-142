...
class BancoA:
    ...
    def depositar(self, user_id, valor):
        with self.lock:
            userService.update_saldo(self.conn, user_id, valor)
            saldo_atual = userService.get_saldo(self.conn, user_id)
    ...
    def retirar(self, user_id, valor):
        with self.lock:
            ...
            if saldo_atual >= valor:
                userService.update_saldo(self.conn, user_id, -valor)
                saldo_atualizado = userService.get_saldo(self.conn, user_id)
    ...
    def definir_chave_pix(self, user_id, chave_pix=None):
        ...
        userService.set_chave_pix(self.conn, user_id, chave_pix)
    ...
    def transferir(self, user_id, chave_pix_destino, valor):
        with self.lock:
            ...
            response = userService.requests.post('http://localhost:5001/transferir', json=payload)
